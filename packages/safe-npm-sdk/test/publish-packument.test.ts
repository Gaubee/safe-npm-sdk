...
describe("buildPublishPackument", () => {
  it("builds a complete packument with computed integrity/shasum", () => {
    const manifest = { name: "my-pkg", version: "1.2.3", description: "a demo" };
    const p = buildPublishPackument(manifest, TARBALL);
  ...
  it("produces stable integrity/shasum for the same input", () => {
    const a = buildPublishPackument({ name: "p", version: "1.0.0" }, TARBALL);
    const b = buildPublishPackument({ name: "p", version: "1.0.0" }, TARBALL);
  ...
  it("handles scoped package names (drops the scope/slash in the tarball filename)", () => {
    const p = buildPublishPackument({ name: "@scope/my-pkg", version: "0.1.0" }, TARBALL);
  ...
  it("respects custom tag, access, and registry", () => {
    const p = buildPublishPackument({ name: "x", version: "2.0.0" }, TARBALL, {
      tag: "beta",
      access: "public",
  ...
  it("produces output that validates against PublishPackumentSchema", () => {
    const p = buildPublishPackument({ name: "valid", version: "1.0.0" }, TARBALL);
